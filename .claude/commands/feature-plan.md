...
- DB スキーマ変更を伴いそうなら `.claude/rules/migrations.md`（採番は `node "$(git rev-parse --show-toplevel)/packages/db/scripts/next-migration-number.mjs"` の出力＝最大+1。既存ファイルのリネーム禁止）と `packages/db/MIGRATIONS.md` を確認する。
