...
以下のどれかに当てはまる場合は、**実装前にレイアウト案・文言案を人間に提示して確認する**。

| 条件 | なぜ確認が必要か |
|------|----------------|
| これまで存在しない新しい通知タイプを追加する | ブランドトーンへの影響を人間が判断する必要がある |
| カラーパレット表に適切な色がなく新色を検討している | パレット拡張はブランド一貫性に関わる意思決定 |
| 複数の通知が連続して送られるフロー（2通以上）を設計する | ユーザー体験上の煩わしさを人間が評価する必要がある |
