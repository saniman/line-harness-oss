...
### 4-6. リマインドが同時刻に2通届く
- **原因**: cron が複数（`*/5 * * * *` と `0 */6 * * *`）あり、00:00/06:00/12:00/18:00 UTC で
  同時発火する。配信処理は「送信後に配信済みマーク」する at-least-once 設計のため、
  2本のcronがほぼ同時に走ると両方が「未配信」と判定して二重送信する。
  前日09:00 JST = 00:00 UTC がちょうど重複時刻に当たっていた。
- **対処**: `scheduled` ハンドラで配信・メンテ系を `*/5 * * * *` 専用にガードする
  （`apps/worker/src/index.ts`、2026-06-17 修正済み）。
- **注意**: サロン予約のリマインダー（`processDueReminders`）も同じ `scheduled` 内にあるため、
  この修正で将来の二重送信リスクも同時に解消されている。
