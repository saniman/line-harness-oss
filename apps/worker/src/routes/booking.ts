...
booking.post('/api/liff/booking/requests', async (c) => {
  ...
  // Fire-and-forget notification + CV — failures must not roll back the booking.
  c.executionCtx.waitUntil(
    Promise.all([
      notifyForBooking(c.env.DB, bookingId, 'requested'),
      fireEvent(c.env.DB, 'booking_requested', {
        friendId,
        eventData: {
          bookingId,
          menuId: body.menu_id,
    ...
    ]).catch((err) => console.error('booking side effects (requested) failed:', err)),
  );
...
booking.patch('/api/booking/admin/requests/:id', async (c) => {
  ...
  const row = await c.env.DB
    .prepare(
      `SELECT id, status, starts_at, friend_id, menu_id, price_at_booking
  ...
  if (next === 'confirmed') {
    ...
    c.executionCtx.waitUntil(
      Promise.all([
        notifyForBooking(c.env.DB, id, 'approved'),
        fireEvent(c.env.DB, 'booking_confirmed', {
      ...
      ]).catch((err) => console.error('booking side effects (confirmed) failed:', err)),
    );
  } else if (next === 'completed') {
    c.executionCtx.waitUntil(
      fireEvent(c.env.DB, 'booking_completed', {
        friendId: row.friend_id,
        eventData: {
          bookingId: id,
          menuId: row.menu_id,
      ...
      }).catch((err) => console.error('booking CV (completed) failed:', err)),
    );
