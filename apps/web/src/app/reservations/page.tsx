...
export default function ReservationsPage() {
  ...
  const handleCancel = async (booking: Booking) => {
    const ok = window.confirm(
      `この予約をキャンセルしますか？\n\n予約者: ${getBookingName(booking)}\n日時: ${formatJST(booking.startAt)}`
    )
    if (!ok) return
    setCancelling(booking.id)
    try {
      await fetchApi(`/api/integrations/google-calendar/bookings/${booking.id}/status`, {
        method: 'PUT',
  ...
  return (
    <div>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
          {/* テーブルヘッダー（デスクトップ） */}
          <div className="hidden sm:grid sm:grid-cols-[1fr_1fr_160px_100px_80px_100px] gap-4 px-4 py-3 bg-gray-50 border-b border-gray-200 text-xs font-semibold text-gray-500 uppercase tracking-wider">
            <span>予約者</span>
            <span>予約日時</span>
            <span>メールアドレス</span>
            <span>ステータス</span>
          ...
          {filtered.map((booking) => {
            ...
            return (
              <div
                key={booking.id}
                className="grid grid-cols-1 sm:grid-cols-[1fr_1fr_160px_100px_80px_100px] gap-1 sm:gap-4 px-4 py-4 border-b border-gray-100 last:border-0 hover:bg-gray-50 transition-colors"
              >
                {/* 予約者 */}
                <div>
